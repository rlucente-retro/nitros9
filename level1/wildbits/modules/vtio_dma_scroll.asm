...
*                    pshs      a                   save it on the stack
*                    lda       #$C2                get the text block #
*                    sta       MAPSLOT             and map it in

 ldb #DMA_CTRL_Enable
 stb >DMA.Base+DMA_CTRL_REG
 ldx #$4000   text screen is at $18_4000 flat address
 tfr x,d
 stb >DMA.Base+DMA_DEST_ADDR_L
 sta >DMA.Base+DMA_DEST_ADDR_M
 ldb #$18
 stb >DMA.Base+DMA_DEST_ADDR_H
 ldb <V.WWidth+1,u  max 255 columns, so this is safe in B
 abx
 tfr x,d
 stb >DMA.Base+DMA_SOURCE_ADDR_L
 sta >DMA.Base+DMA_SOURCE_ADDR_M
 ldb #$18
 stb >DMA.Base+DMA_SOURCE_ADDR_H
 ldd <V.WWidth,u          get screen width in A and height in B
 decb                          decrement height by 1
 mul                           get the product (bytes to copy)
 stb >DMA.Base+DMA_SIZE_1D_L
 sta >DMA.Base+DMA_SIZE_1D_M
 clr >DMA.Base+DMA_SIZE_1D_H
 ldb >DMA.Base+DMA_CTRL_REG
 orb #DMA_CTRL_Start_Trf
 stb >DMA.Base+DMA_CTRL_REG

w@ LDA >DMA.Base+DMA_STATUS_REG
        ANDA    #DMA_STATUS_TRF_IP
        CMPA    #DMA_STATUS_TRF_IP
        BNE     w@
